...
uint8_t i2c_send(uint8_t data)
{
    ...
    /* Send Data */
    TRISSDA = 0;                        // Store Output Mode
    BitPos = 0x80;                      // Initialize Bit Position
    for(i = 0; i < 8; i++)              // Repeat 8bit
    {
        SCL = 0;                        // SCL Low
        ...
        SCL = 1;                        // Restore SCL High
    }
    
    /* Check ACK */
    SCL = 0;                            // SCL Low for One CLOCK
    TRISSDA = 1;                        // Store Input Mode
    __delay_us(2);
    SCL = 1;                            // Restore SCL High
    __delay_us(2);
    ACK = SDA;                          // Check ACK
